...
namespace FOC.Tests
{
    public sealed class AIDecisionMakingTests
    {
        ...
        {
            var measured=MeasureScheduler(count,aggregate);WritePerformance(count,aggregate,measured);Assert.That(measured.Run.Results,Has.Count.EqualTo(count));Assert.That(measured.Run.Metrics.SelectedActions,Is.EqualTo(count));
            if(aggregate){var individual=MeasureScheduler(count,false);WritePerformance(count,false,individual);Assert.That(measured.Run.Results.Select(x=>x.Proposal!.Candidate.Id),Is.EqualTo(individual.Run.Results.Select(x=>x.Proposal!.Candidate.Id)));Assert.That(measured.Run.Metrics.ContextBuilds,Is.LessThan(individual.Run.Metrics.ContextBuilds));Assert.That(measured.Run.Metrics.CandidateGenerations,Is.LessThan(individual.Run.Metrics.CandidateGenerations));}
        }
        ...
        private static (AISchedulerRun Run,double Milliseconds,long Allocated) MeasureScheduler(int count,bool aggregate){var beforeBytes=GC.GetTotalMemory(true);var timer=Stopwatch.StartNew();var run=new AIDecisionScheduler().Run(new WorldTimestamp(10),Controllers(count,aggregate),ProofAIContent.CreateCatalog(),new ProofScheduledSource(aggregate),new SingleCandidateProvider(),new[]{new AllowPolicy()},new[]{new CandidateUtilityPolicy(1,1)});timer.Stop();return(run,timer.Elapsed.TotalMilliseconds,Math.Max(0,GC.GetTotalMemory(false)-beforeBytes));}
        private static void WritePerformance(int count,bool aggregate,(AISchedulerRun Run,double Milliseconds,long Allocated) measured){TestContext.Progress.WriteLine("AI_PERFORMANCE owners={0} mode={1} elapsed_ms={2:F3} context_builds={3} candidate_generations={4} utility_evaluations={5} shared={6} selected={7} managed_delta_bytes={8}",count,aggregate?"aggregated":"individual",measured.Milliseconds,measured.Run.Metrics.ContextBuilds,measured.Run.Metrics.CandidateGenerations,measured.Run.Metrics.UtilityEvaluations,measured.Run.Metrics.SharedOrAggregatedEvaluations,measured.Run.Metrics.SelectedActions,measured.Allocated);}
