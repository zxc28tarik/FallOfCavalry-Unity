...
namespace FOC.Presentation.Core
{
    ...
    public sealed class CampaignPresentationQueries
    {
        ...
        public MapReadModel BuildMap(PresentationViewerContext viewer, IMapPresentationDataProvider provider)
        {
            ...
            var fields = new[] { Field("presentation.map.slice", "İstanbul · Marmara · Trakya", PresentationKnowledge.ExactSelf), Field("presentation.map.known-markers", markers.Count.ToString(), PresentationKnowledge.ExactSelf), Field("presentation.map.routes", routes.Count.ToString(), PresentationKnowledge.ExactSelf), Field("presentation.map.active-journeys", journeys.Count.ToString(), PresentationKnowledge.ExactSelf), Field("presentation.map.selected", "İstanbul", PresentationKnowledge.ExactSelf, Entity(PresentationEntityKind.City,"city-istanbul")) };
        ...
        public ArmyReadModel BuildArmy(PresentationViewerContext viewer, ArmyId armyId)
        {
            ...
            var fields = new[]
            {
                ...
                Field("presentation.army.owner", "presentation.army-owner." + Key(army.Owner.Kind) + ":" + army.Owner.Id, PresentationKnowledge.ExactSelf, army.Owner.Kind == ArmyOwnerKind.Character ? Entity(PresentationEntityKind.Character, army.Owner.Id) : (PresentationEntityRef?)null),
                Field("presentation.army.controller", "presentation.army-owner." + Key(army.Controller.Kind) + ":" + army.Controller.Id, PresentationKnowledge.ExactSelf, army.Controller.Kind == ArmyOwnerKind.Character ? Entity(PresentationEntityKind.Character, army.Controller.Id) : (PresentationEntityRef?)null),
