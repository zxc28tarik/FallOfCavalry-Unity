...
namespace FOC.Presentation.Visuals
{
    ...
    public sealed class VisualCatalogAsset:ScriptableObject
    {
        ...
        [SerializeField]private List<AssetEntry> assets=new List<AssetEntry>();[SerializeField]private List<ProfileEntry> profiles=new List<ProfileEntry>();[SerializeField]private List<EquipmentEntry> equipment=new List<EquipmentEntry>();private Dictionary<string,GameObject>? prefabLookup;
        public IReadOnlyList<AssetEntry> Assets=>assets;public IReadOnlyList<ProfileEntry> Profiles=>profiles;public IReadOnlyList<EquipmentEntry> Equipment=>equipment;
        public void ReplaceContent(IEnumerable<AssetEntry> newAssets,IEnumerable<ProfileEntry> newProfiles,IEnumerable<EquipmentEntry> newEquipment){assets=new List<AssetEntry>(newAssets??throw new ArgumentNullException(nameof(newAssets)));profiles=new List<ProfileEntry>(newProfiles??throw new ArgumentNullException(nameof(newProfiles)));equipment=new List<EquipmentEntry>(newEquipment??throw new ArgumentNullException(nameof(newEquipment)));prefabLookup=null;}
