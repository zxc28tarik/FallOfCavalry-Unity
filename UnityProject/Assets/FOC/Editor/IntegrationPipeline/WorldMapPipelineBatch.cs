    ...
    public static class WorldMapPipelineBatch
    {
        public static void Run()
        {
            try
            {
                ...
                var historical=Resources.Load<TextAsset>("FOC/HistoricalSlice/historical-slice-content")??throw new InvalidOperationException("Historical content missing.");var campaign=VerticalSliceCampaignFactory.Create(locations.text,routes.text,historical.text);if(!new GeographyInvariantValidator().Validate(campaign).IsValid)throw new InvalidOperationException("Runtime geography invalid.");var save=CampaignSaveMapper.ToSaveData(campaign);if(!new CampaignSaveValidator().Validate(save).IsValid)throw new InvalidOperationException("Save geography invalid.");var serializer=new CampaignSaveTextSerializer();var read=serializer.Deserialize(serializer.Serialize(save));if(!read.Success||read.Data==null)throw new InvalidOperationException(read.Error);CampaignSaveMapper.ToRuntimeState(read.Data);
                var map=new WorldMapPresentationDataProvider(campaign);if(map.GetKnownMarkers(new PresentationViewerContext(FOC.Domain.Common.FactionId.Create("faction-ottoman-state"),new PresentationEntityRef(PresentationEntityKind.Character,"hasan-aga"))).Count<12)throw new InvalidOperationException("Map marker projection failed.");
