...
Automated smoke launches the real executable with `-focSmokeTest`. It waits for UI startup, executes Save then Load through the same coordinator, emits `FOC_DEVELOPMENT_BOOTSTRAP_READY`, `FOC_SAVE_UI SAVED`, `FOC_SAVE_UI LOADED` and `FOC_DEVELOPMENT_SMOKE_PASS`, then exits 0. The validated artifact total was 153,939,362 bytes; executable SHA-256 was `080EAE12A6B1C874F8B5C96E3D22CEB1F8A278017507A46901DBE0B6396CFD1B`.
