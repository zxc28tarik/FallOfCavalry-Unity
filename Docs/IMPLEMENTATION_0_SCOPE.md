...
2. User authority clarification dated 2026-09-08: Transfer Package Parts 1–11 are complete and binding operational authority for the current work when standalone Master/P1–P15/Decision Ledger files are unavailable.
3. FOC Master Design Authority, final P1–P15 documents, and current Decision Ledger: no standalone copies were found in the inspected locations. Their absence does not block Implementation 0.
4. Transfer Package Parts 1–11, preserved in the desktop transfer archive, are the active implementation authority.
5. Legacy JS/Phaser code is reference only and never overrides the transfer package.

Absence of a standalone Master file does not authorize invention of gameplay semantics and is not an Implementation 0 blocker. If higher-authority documents are later added, the repository must receive a conflict audit; the newest explicit user decision always wins.
